...
public class Server implements Container {
   ...
   public Server(String rootPath, String debugLevel) throws Exception  {
      this(new File(rootPath), debugLevel);          
   }
   
   public Server(File rootPath, String debugLevel) throws Exception  {
     // System.setErr(new PrintStream(new FileOutputStream(new File(rootPath, "performance.log"))));
      this.bean = ManagementFactory.getThreadMXBean();
      this.rootPath = rootPath;
      this.level = Enum.valueOf(DebugLevel.class, debugLevel);
   }
   ...
   public void process(Request request, Response response) throws Exception {
      ...
      System.err.println(Thread.currentThread().getId() +" OUTPUTSTREAM TIME="+(System.currentTimeMillis()-start)+" CPU="+(TimeUnit.MILLISECONDS.convert(bean.getCurrentThreadCpuTime() - cpuTime, TimeUnit.NANOSECONDS)));
      String target = request.getPath().getPath();
      //String target = request.getTarget();
      System.err.println(Thread.currentThread().getId() +"["+target+"] PATH TIME="+(System.currentTimeMillis()-start)+" CPU="+(TimeUnit.MILLISECONDS.convert(bean.getCurrentThreadCpuTime() - cpuTime, TimeUnit.NANOSECONDS)));
      String filePath = target.replace('/', File.separatorChar);
      System.err.println(Thread.currentThread().getId() +" REPLACE TIME="+(System.currentTimeMillis()-start)+" CPU="+(TimeUnit.MILLISECONDS.convert(bean.getCurrentThreadCpuTime() - cpuTime, TimeUnit.NANOSECONDS)));
      String type = "text/html; charset=utf-8";
      byte[] content = null;
      System.err.println(""+Thread.currentThread().getId() +" HEADER TIME="+(System.currentTimeMillis()-start)+" CPU="+(TimeUnit.MILLISECONDS.convert(bean.getCurrentThreadCpuTime() - cpuTime, TimeUnit.NANOSECONDS)));
      long finished = bean.getCurrentThreadCpuTime();
      System.err.printf("BEFORE=[%s] AFTER=[%s]%n", cpuTime, finished);
      start = System.currentTimeMillis();
      if(new File(rootPath, filePath).isDirectory()) {
         content = new Directory().getContents(rootPath, target);

         response.set("Content-Type", "text/html; charset=utf-8");
         response.set("Content-Length", content.length);
         out.write(content);
      } else {
         byte[] cached = fileCache.get(filePath);
         
         File realFile = new File(rootPath, filePath);
         response.set("Content-Type", Indexer.getContentType(target));
         response.set("Content-Length", (int)realFile.length());
         response.set("Connection", "close");
         
         System.err.println(""+Thread.currentThread().getId() +" INDEX TIME="+(System.currentTimeMillis()-start));
         start = System.currentTimeMillis();
         
         if(cached == null) {
            InputStream file = new FileInputStream(realFile);       
            ...
            cached = chunk;
            fileCache.put(filePath, cached);
            
            System.err.println(""+Thread.currentThread().getId() +" ### FILL TIME="+(System.currentTimeMillis()-start));
            start = System.currentTimeMillis();
         }
         content = cached;
         out.write(cached);
      ...
      System.err.println(""+Thread.currentThread().getId() +" WRITE TIME="+(System.currentTimeMillis()-start));
      start = System.currentTimeMillis();
      out.close(); 

      System.err.println(""+Thread.currentThread().getId() +" CLOSE TIME="+(System.currentTimeMillis()-start));
      System.err.println(request);
      System.err.println(response);
      
      if(level.isEnabled(DebugLevel.HIGH)) {
   ...
   public void init() throws IOException {
      if(!rootPath.exists()) {
         throw new IOException("Could not create root path");
      }
      System.err.println("Using rootPath=["+rootPath+"]");
   ...
   public static void main(String[] list) throws Exception {
           /*
      FileOutputStream out = new FileOutputStream("c:\\debug");
      ...
      */
      if(list.length < 4) {
         list = new String[]{
               "/Users/niall/Workspace/http/application/Server",
               "9090",
               "9091",
               "TLS",
               "HIGH"
      ...
      SSLContext sslContext = SSLContext.getInstance(list[3]); // SSLv3 TLS
      ...
      Server server = new Server(list[0], list[4]);
      ContainerServer processor = new ContainerServer(server, 5);
      org.simpleframework.transport.Server secure = new SecureProcessor(processor, sslContext);
      SocketConnection connection = new SocketConnection(processor);
      SocketConnection ssl = new SocketConnection(secure);
      
      connection.connect(new InetSocketAddress(Integer.parseInt(list[1])));
      ssl.connect(new InetSocketAddress(Integer.parseInt(list[2])));
