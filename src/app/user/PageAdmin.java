...
public class PageAdmin {
    private UserComponent component; // our host component
    ...
    public String handleAdminUser(Request request, String result){
      ...
      // edit a user
        ...
        if(utils.text.isEmpty(edit)==false){
            return showPageEdit(edit) + logOutText();
        }
      ...
      // handle a request for user details
        ...
        String output = "<h2>Edit users</h2>\n"
            + "<table style=\"height: 156px; width: 995px;\">\n"
            + "<tbody style=\"vertical-align: top;\">\n"

            + "<tr>"
            +"      <td style=\"text-align: left; width: 86px;\">"
            +"      <h3 style=\"width: 114px;\">User</h3>"
            +"      </td>"
            +"      <td style=\"text-align: left; width: 149px;\" valign=\"undefined\">"
            +"      <h3 style=\"width: 107px;\">Actions</h3>"
            +"      </td>"
            +"      <td style=\"width: 301px;\" align=\"undefined\" valign=\"undefined\">"
            +"      <h3 style=\"margin-left: 0px; width: 375px;\">Permissions</h3>"
            +"      </td>"
            +"      <td style=\"width: 79px; text-align: left;\" valign=\"undefined\">"
            +"      <h3>Status</h3>"
            +"      </td>"
        ...
        for(User user : component.getUsers()){
            // create the entry for this user
             String converted =
                "<tr>"
                +"      <td style=\"width: 86px;\"><tt>"+user.getMyName()+"</tt></td>"
                +"      <td"
                +" style=\"text-align: left; font-family: monospace; width: 149px;\""
                +" valign=\"undefined\">"
                +"<a href=\""+component.getWebAddress()+"?details="+user.getMyName()+"\">details</a>|"
                +"<a href=\""+component.getWebAddress()+"?edit="+user.getMyName()+"\">edit</a>|"
                +"<a href=\""+component.getWebAddress()+"?"+User.doDelete+"="+user.getMyName()+"\">delete</a>"
                + "</td>"
                +"      <td style=\"width: 301px; font-family: monospace;\""
                +" align=\"undefined\" valign=\"undefined\">"+user.getMyPermissions()+"</td>"
                +"      <td style=\"width: 79px; text-align: left;\""
                +" valign=\"undefined\"><span style=\"font-family: monospace;\">active</span></td>"
                +"      <td style=\"width: 229px;\" align=\"undefined\""
                +" valign=\"undefined\"><span style=\"font-family: monospace;\">"
    ...
    /** Show the page of details about a given user */
    private String doDetails(String who){
        // get the user that they relate to
        User user = component.getUser(who);
        // does this user exists?
        if(user == null){
...
//        component.deleteUser(who);
    ...
    /** Show the page of details about a given user */
    private String showPageEdit(String who){
