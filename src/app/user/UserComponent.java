...
public class UserComponent extends Component{
  ...
  // show logout - this only appears if we are already logged
        if(this.isLogged(request)){
            ...
            result = this.pageAdmin.showAdminPage(request, result);
  ...
  // show login: If we don't want anything else, just proceed with the login
            result = getTextLogin(request);
            result = pageAdmin.showAdminPage(request, result);
            this.html.setSection(html.SectionHome, result);
        }
    ...
    private String processLogin(Request request, Response response){
        ...
        // get the user details from the form
         ...
         if(hasLogged){
            ...
            // redirection tags, default to root
            String reDirection = "/";
            // do we have a redirection tag?
            String action = utils.internet.getHTMLparameter(request, "redirect");
            // should we redirect elsewhere?
            if(utils.text.isEmpty(action)==false){
                reDirection = action;
            }
            
            // redirect to the main page
            addAutoHTMLrefresh(3, reDirection);
            return "Valid login";
         }
         // someting went wrong
         else{
...
//        System.out.println(loginStatus + "--->" + user.getMyName());
    ...
    private String getTextLogin(Request request){
       
        // redirection holder
        String doCustomization = "";
        // should we redirect after login?
        String action = utils.internet.getHTMLparameter(request, "redirect");
        // check if yes or no
        if(utils.text.isEmpty(action)==false){
            doCustomization = "&redirect=" + action;
        }
    ...
    return
        ...
        +"<form name=\"login\" action=\"?action=doLogin"+doCustomization+"\""
        +" method=\"post\"><br>"
